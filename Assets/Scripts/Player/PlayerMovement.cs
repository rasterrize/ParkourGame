...
namespace Player
{
    public class PlayerMovement : MonoBehaviour
    {
        ...
        [Tooltip("The gravity of the player. This defaults to earth's gravity.")] [SerializeField]
        private float gravity = DEFAULT_GRAVITY;

        [Tooltip("The walking speed of the player.")] [SerializeField]
        private float walkSpeed = DEFAULT_WALK_SPEED;

        [Tooltip("The running speed of the player.")] [SerializeField]
        private float runSpeed = DEFAULT_RUN_SPEED;

        [Tooltip("The amount of force to apply when the player jumps")] [SerializeField]
        private float jumpForce = DEFAULT_JUMP_FORCE;

        [Tooltip("The speed at which sliding will start at.")] [SerializeField]
        private float slideStartVelocity = DEFAULT_SLIDE_START_VELOCITY;

        [Tooltip("The speed at which sliding will end at automatically.")] [SerializeField]
        private float slideEndVelocity = DEFAULT_SLIDE_END_VELOCITY;

        [Tooltip("The maximum speed the player can slide. Usually only reached when sliding downhill.")]
        [SerializeField]
        private float maxSlideSpeed = DEFAULT_SLIDE_MAX_SPEED;

        [Tooltip("The distance to move the camera down on the y axis when the player slides.")] [SerializeField]
        private float slideCameraYDistance = DEFAULT_SLIDE_CAMERA_Y_DISTANCE;

        [Tooltip("The penalty multiplier for consecutive slides.")] [SerializeField]
        private float slidePenaltyMultiplier = DEFAULT_SLIDE_PENALTY_MULTIPLIER;

        [Tooltip("The speed at which the player will run on walls")] [SerializeField]
        private float wallRunSpeed = DEFAULT_WALL_RUN_SPEED;

        [Tooltip("The distance for player to wall checks to initiate mechanics such as wall running.")] [SerializeField]
        private float wallCheckDistance = DEFAULT_WALL_CHECK_DISTANCE;
        ...
        private void RunGroundedCheck()
        {
            ...
            IsGrounded = useCustomGroundCheck ? customGroundCheck.IsGrounded() : CharController.isGrounded;
        ...
        {
            // Left Wall Check
            if (Physics.Raycast(wallCheckLeft.transform.position, -wallCheckLeft.transform.right, out RaycastHit hit,
                    wallCheckDistance, parkourWallLayer))
            {
            ...
            // Right Wall Check
            if (Physics.Raycast(wallCheckRight.transform.position, wallCheckRight.transform.right, out RaycastHit hit2,
                    wallCheckDistance, parkourWallLayer))
            {
                if (currentMovementState.GetType() != typeof(WallRunState) && !WaitUntilFarFromWall)
